...
public class ProjectsCLI {
	...
	public ProjectsCLI() {
		...
		dataSource.setUrl("jdbc:postgresql://localhost:5432/apireview");  // this has the name of the data base to access
...
//		List<Department> allDepartments = departmentDAO.getAllDepartments();
		...
		RestTemplate callApi = new RestTemplate();   // instantiate a RestTemplate to mange our API calls

		Department[] deptArray;           // Hold the results from API call as an array
		                                  // This needed RestTemplate has no easy way to return a List

		// Call the api to get the data back as an array of Department objects
		deptArray = callApi.getForObject("http://localhost:8080/departments", Department[].class);
		
		// Call the method to display with the Department array convert to a List using the Array.asList()
