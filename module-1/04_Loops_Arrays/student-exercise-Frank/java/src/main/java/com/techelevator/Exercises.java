...
public class Exercises {
	...
	public boolean firstLast6(int[] nums) {  // receives an array of ints called nums
		...
		// look at the first and last element in the array and if it's 6 return true, otherwise false
		// first element is at index 0
		// last element is at index array.length - 1
		
		int lastElement = nums.length - 1;  // Store teh last valid index in a variable
...
//		}
	 ...
	 */
	public boolean sameFirstLast(int[] nums) { // receives an int array called nums
		
		// Check to see if array passed has at least one element in it - if not, return false
		
		if (nums.length < 1) {  // nums.length - number of elements in the array
			return false;
		}
		
		// Compare the first and last elements to see if they are equal, if they are - return true
		//                                                               if not - return false
		//  1st-Elem    last-elem
	...
	public int[] makePi() {  // receives no parameters and returns an int array
	...
	/*
	 ...
	 sum3([7, 0, 0]) → 7
	 */
	public int sum3(int[] nums) {  // receives an array of int called nums
		// Sum all numbers in array and return sum
	
		// Define a variable to hold the sum so I return it
		int sum = 0;  // hold the sum all the numbers in the array
		
		// Go through the array one element at a time and add it to a sum
		// Since I know how many elements are in the array, I can use a for-loop to go through the array
		// number-elements-in-array = arrayName.length
		
		for (int i=0; i < nums.length; i++) {  // the only thing that changes is the array name
			sum = sum + nums[i];     // add the current element in the array to sum
		}
		
		return sum;  // return the variable holding the sum of the values
	}
