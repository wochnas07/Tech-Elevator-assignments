...
public class ItalianPlayingCard extends PlayingCard {
	...
	public ItalianPlayingCard() 
	{
		...
		initializeMaps();  // initializeMaps() will be run when this ctor is run even if the Maps are already initialized
	} 
	...
	public ItalianPlayingCard(int value, String suit) {
		...
		if (value > MAXVALUE) {
			this.value = MAXVALUE;   // set the current object called value to MAXVALUE
		}
		if (value < MINVALUE) {
			setValue(MINVALUE);
		}
